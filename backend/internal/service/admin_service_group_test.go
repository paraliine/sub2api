...
func TestAdminService_UpdateGroup_LimitFieldsPartialUpdate(t *testing.T) {
	fiveHour, daily, weekly, monthly := 5.0, 10.0, 20.0, 30.0
	existingGroup := &Group{
		ID:               1,
		Name:             "existing-group",
		Platform:         PlatformOpenAI,
	...
	t.Run("non-quota update preserves all limits", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, 5.0, *group.FiveHourLimitUSD)
	...
	t.Run("five-hour limit supports zero and explicit unlimited", func(t *testing.T) {
		for _, limit := range []float64{0, 8, -1} {
			group, err := svc.UpdateGroup(context.Background(), existingGroup.ID, &UpdateGroupInput{
				FiveHourLimitUSD: &limit,
			})
			require.NoError(t, err)
			if limit < 0 {
				require.Nil(t, group.FiveHourLimitUSD)
			} else {
				require.Equal(t, limit, *group.FiveHourLimitUSD)
			}
			require.Equal(t, 15.0, *group.DailyLimitUSD)
			require.Nil(t, group.WeeklyLimitUSD)
			require.Equal(t, 30.0, *group.MonthlyLimitUSD)
