...
func TestOpenAIGatewayService_CodexFingerprintCompactProjectsIdentityWithoutClientMetadata(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, wantThread, upstream.lastReq.Header.Get("thread-id"))
	require.Equal(t, wantThread+":0", upstream.lastReq.Header.Get("x-codex-window-id"))
	require.NotSame(t, staleIDs, stagedCodexFingerprintIDs(c, account))
}
